...
things Supernova doesn't handle natively. Most often SMG2 hack extensions that introduce their
own BCSV tables or added fields.
...
handled by the host), they just emit widgets. Exceptions thrown from any plugin method are caught and
...
1. Build your plugin (`dotnet build -c Release` from the plugin's own folder).
2. Copy the plugin's `.dll` (and any private dependency DLLs) into the `plugins/` folder that
   sits **next to the Supernova executable**:
   - a downloaded build: the `plugins/` folder inside the extracted zip;
   - `dotnet run --project src/SMGEditor.Editor` or the dev build: `src/SMGEditor.Editor/bin/<Config>/net8.0/plugins/`.

   Supernova creates that folder on first run if it is missing.
3. Start Supernova, click **Settings** on the galaxy list, tick your plugin.
4. Open a galaxy, then **Tools > Plugins > (your plugin)**.

Adding or replacing a plugin file needs a restart. Enabling and disabling does not.
